...
class IVM_functions:
	...
	def get_stuff(console_url, creds, resource):
		...
		response = requests.request("GET", url, headers=headers, data=payload, verify=False, timeout=90)
	...
	def post_stuff(console_url, creds, resource, in_payload):
		...
		response = requests.request("POST", url, headers=headers, data=payload, verify=False, timeout=90)
		json_text = response.text
		parsed = json.loads(json_text)
	...
	def put_stuff(console_url, creds, resource, in_payload):
		...
		response = requests.request("PUT", url, headers=headers, data=payload, verify=False, timeout=90)
		json_text = response.text
		parsed = json.loads(json_text)
		return parsed