    ...
    def filter_empty_sites(self) -> List[Dict[str, Any]]:
        ...
        for site in all_sites:
            ...
            try:
                ...
                logger.warning(
                    "Could not check assets for site %s: %s", site_id, e
                )
    ...
    def filter_by_ids(self, site_ids: List[int]) -> List[Dict[str, Any]]:
        ...
        for site_id in site_ids:
            try:
                ...
                logger.warning("Could not retrieve site %s: %s", site_id, e)
            ...
            try:
                ...
                if not dry_run:
                    try:
                        ...
                        logger.info(
                            "Deleted site %s: %s", site_id, site.get('name')
                        )
                        ...
                        logger.error(
                            "Failed to delete site %s: %s", site_id, delete_error
                        )
                ...
                result['failures'].append(error_info)  # type: ignore
                result['failure_count'] += 1  # type: ignore
                logger.error("Could not retrieve site %s: %s", site_id, e)
